...
// Time-stamp: <2024-12-27 22:41:55 krylon>
...
const minSize = 4096 // 4 KiB, one page of virtual memory
...
func (w *Walker) visit(path string, info fs.FileInfo, incoming error) error {
	...
	if incoming != nil {
		...
		w.log.Printf("[TRACE] Add file %s to database...\n",
			path)
		f = &model.File{Path: path, CTime: info.ModTime()}
		...
		w.db.FileUpdateCtime(f, info.ModTime())
	}
